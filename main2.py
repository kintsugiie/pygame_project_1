...
for planet_name in data['planets']:
    ...
    color_hex = planet_data['color']
    color_rgb = tuple(int(color_hex[i:i+2], 16) for i in (1, 3, 5)) if color_hex.startswith('#') else (255, 255, 255)
...
while run:
    ...
    for sprite in all_sprites.sprites():
        if hasattr(sprite, 'distance'):  # проверяем, что это планета
